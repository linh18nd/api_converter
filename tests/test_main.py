...
class TestMain:
    ...
    @pytest.mark.parametrize(
        "test_input,expected", [("123456", 200), ("invalid", 403)]
    )
    ...
    def test_get_doc_detail(self, monkeypatch, document_model, client):
        ...
        response = client.get(f"/ocr/{document.pid}", headers={"X-API-KEY": "123456"})
    ...
    def test_get_doc_pdf(self, monkeypatch, document_model, client):
        ...
        response = client.get(
            f"/ocr/{document.pid}/pdf", headers={"X-API-KEY": "123456"}
        )
    ...
    def test_get_doc_txt(self, monkeypatch, document_model, client):
        ...
        response = client.get(
            f"/ocr/{document.pid}/txt", headers={"X-API-KEY": "123456"}
        )
